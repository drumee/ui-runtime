...
| `src/websocket.js::Websocket` | `sources/ui-team/src/drumee/router/websocket/index.js::{connect,resetSocket,onmessage,ping,bindEvent,unbindEvent,upstream}` | `0fb6fe4953281cf9f53b87b4d0712bd41c8cf4c3` | Retains native browser `service` connection, JSON service envelope dispatch, generic bind/unbind, array upstream, 5-second reconnect, capped retry and keepalive. Phase 4's real cookie Upgrade replaces Team `bootstrap.authn` token acquisition; Team radios, Visitor/Butler routing, service maps and Wm state are removed. Unit tests cover envelope, bind/unbind, reconnect and keepalive; browser E2E proves real connection and DOM update. |
